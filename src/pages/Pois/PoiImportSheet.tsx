...
import { POI_CATEGORY_LABEL } from "@constants/poi";
...
import {
    ...
    uploadPoiImportFile,
    commitPoiImport,
    downloadPoiImportTemplate,
    downloadImportJobErrors,
    pollImportJobUntilSettled,
} from "@service/importApi";

interface PoiImportSheetProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    onImported: () => void;
}

const PoiImportSheet: React.FC<PoiImportSheetProps> = ({
    open,
    onOpenChange,
    onImported,
}) => {
    const [file, setFile] = useState<File | null>(null);
    const [job, setJob] = useState<ImportJob<PoiImportPreviewRow> | null>(null);
    ...
    const handleDownloadTemplate = async () => {
        try {
            ...
            await downloadPoiImportTemplate();
        } catch (err) {
    ...
    const handleUpload = async (overrideSheetName?: string) => {
        ...
        try {
            setUploading(true);
            const result = await uploadPoiImportFile(
                file,
    ...
    const doCommit = async () => {
        ...
        try {
            setCommitting(true);
            setConfirmOpen(false);
            await commitPoiImport(job._id);
            const result = await pollImportJobUntilSettled<PoiImportPreviewRow>(
                job._id,
                setJob,
            );
            ...
            toast.success(`Đã nhập thành công ${result.createdCount} điểm tiện ích`);
            reset();
            onOpenChange(false);
            onImported();
        } catch (err) {
    ...
    const handleExportErrors = async () => {
        ...
        try {
            ...
            await downloadImportJobErrors(job._id, "import-loi-diem-tien-ich.xlsx");
        } catch (err) {
            toast.error((err as AppError).message);
        } finally {
    ...
    return (
        <Sheet open={open} onOpenChange={handleOpenChange}>
            <SheetContent className="flex w-full flex-col sm:max-w-3xl lg:max-w-[calc(100vw-320px)]">
                <SheetHeader>
                    <SheetTitle>Nhập điểm tiện ích từ Excel</SheetTitle>
                </SheetHeader>
                    ...
                    {!job && (
                        <>
                            <div className="rounded-lg border border-divider_01 bg-surface_2 p-3 text-xs text-text_2">
                                ...
                                tiên, với các cột khớp đúng tên: &quot;Tên&quot;,
                                &quot;Danh mục&quot;, &quot;Vĩ độ&quot;,
                                &quot;Kinh độ&quot;, &quot;Địa chỉ&quot;,
                                &quot;Đã duyệt&quot; — tải mẫu bên dưới để đảm
                                bảo đúng định dạng. Danh mục &quot;Hộ
                                dân&quot; không nhập được qua Excel (dùng công
                                cụ &quot;Gắn hộ dân lên bản đồ&quot;). Bỏ
                                trống &quot;Đã duyệt&quot; sẽ mặc định là đã
                                duyệt (hiện luôn trên Bản đồ tiện ích).
                            </div>
                            <Button
                                type="button"
                                variant="outline"
                    ...
                    {job && job.availableSheetNames.length > 1 && (
                        <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                            <p>
                                ...
                                Nếu đây không phải sheet chứa dữ liệu điểm
                                tiện ích, chọn đúng sheet bên dưới rồi tải lên
                                lại.
                            </p>
                                    ...
                                    <TableBody>
                                        {job.previewData.map((row, idx) => (
                                            ...
                                            <TableRow key={`${row.name}-${idx}`}>
                                                ...
                                                <TableCell>
                                                    {POI_CATEGORY_LABEL[row.category] ||
                                                        row.category}
                                                </TableCell>
                                                <TableCell>
                                                    {row.lat}, {row.lng}
                ...
                <SheetFooter className="flex-col gap-2">
                    ...
                    {job && (
                        ...
                        >
                            Xác nhận nhập {job.validRows} điểm tiện ích
                        </Button>
                    )}
                </SheetFooter>
            </SheetContent>
