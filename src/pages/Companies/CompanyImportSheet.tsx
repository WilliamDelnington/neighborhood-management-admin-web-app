...
interface CompanyImportSheetProps {
    open: boolean;
...
type MappingForm = Record<keyof CompanyColumnMapping, string>;

const EMPTY_MAPPING: MappingForm = COMPANY_MAPPING_FIELDS.reduce(
    (acc, f) => ({ ...acc, [f.key]: "" }),
    {} as MappingForm,
);

const CompanyImportSheet: React.FC<CompanyImportSheetProps> = ({
    open,
    onOpenChange,
    onImported,
}) => {
    const [file, setFile] = useState<File | null>(null);
    const [job, setJob] = useState<ImportJob<CompanyImportPreviewRow> | null>(
        null,
    );
    ...
    const handleUpload = async (overrideSheetName?: string) => {
        ...
        try {
            setUploading(true);
            const result = await uploadCompanyImportFile(
                file,
    ...
    const handleApplyMapping = async () => {
        ...
        try {
            ...
            const payload: Partial<
                Record<keyof CompanyColumnMapping, string>
            > = {
                name: mapping.name,
            ...
            const result = await applyCompanyImportMapping(
                job._id,
                payload as CompanyColumnMapping,
            );
            setJob(result);
            setShowMapping(false);
    ...
    const doCommit = async () => {
        ...
        try {
            setCommitting(true);
            setConfirmOpen(false);
            await commitCompanyImport(job._id);
            // Backend chuyen job sang "committing" va xu ly tung dong o
            // background (xem processCompanyImportRows) - poll de cap nhat
            // thanh tien do (progress bar) thay vi cho 1 request duy nhat,
            // tranh timeout khi import nhieu du lieu.
            const result = await pollImportJobUntilSettled<CompanyImportPreviewRow>(
                job._id,
                setJob,
            );
            ...
            toast.success(`Đã nhập thành công ${result.createdCount} công ty`);
            reset();
            onOpenChange(false);
            onImported();
        } catch (err) {
    ...
    const handleExportErrors = async () => {
        ...
        try {
            ...
            await downloadImportJobErrors(job._id, "import-loi-cong-ty.xlsx");
        } catch (err) {
            toast.error((err as AppError).message);
        } finally {
    ...
    return (
        <Sheet open={open} onOpenChange={handleOpenChange}>
            <SheetContent className="flex w-full flex-col sm:max-w-3xl lg:max-w-[calc(100vw-320px)]">
                <SheetHeader>
                    <SheetTitle>Nhập công ty từ Excel</SheetTitle>
                </SheetHeader>
                    ...
                    {job && job.availableSheetNames.length > 1 && (
                        <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                            <p>
                                ...
                                Nếu đây không phải sheet chứa dữ liệu công ty,
                ...
                <SheetFooter className="flex-col gap-2">
                    ...
                    {job && !showMapping && (
                        ...
                        >
                            Xác nhận nhập {job.validRows} công ty
                        </Button>
                    )}
                </SheetFooter>
            </SheetContent>
...
export default CompanyImportSheet;
