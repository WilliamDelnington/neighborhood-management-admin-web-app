...
import {
    NeighborhoodMemberColumnMapping,
    NeighborhoodMemberImportPreviewRow,
    ImportJob,
    uploadNeighborhoodMemberImportFile,
    applyNeighborhoodMemberImportMapping,
    commitNeighborhoodMemberImport,
    downloadNeighborhoodMemberImportTemplate,
    downloadImportJobErrors,
    pollImportJobUntilSettled,
} from "@service/importApi";

interface NeighborhoodMemberImportSheetProps {
...
// "phone"/"roleName" bat buoc - "note" tuy chon (bo qua = khong dung cot
// nao, xem neighborhoodMemberImportMappingSchema o backend). Khong ho tro
// chon pham vi con (street/house-group/campaign) qua Excel - dong vai tro
// Cong tac vien luon mac dinh ca To dan pho, ai can pham vi hep hon dung
// form phan cong thu cong tren panel.
const NEIGHBORHOOD_MEMBER_MAPPING_FIELDS: {
    key: keyof NeighborhoodMemberColumnMapping;
    label: string;
    required?: boolean;
}[] = [
    { key: "phone", label: "Số điện thoại", required: true },
    { key: "roleName", label: "Vai trò", required: true },
    { key: "note", label: "Ghi chú" },
];

type MappingForm = Record<keyof NeighborhoodMemberColumnMapping, string>;

const EMPTY_MAPPING: MappingForm = NEIGHBORHOOD_MEMBER_MAPPING_FIELDS.reduce(
    (acc, f) => ({ ...acc, [f.key]: "" }),
    {} as MappingForm,
);

const NeighborhoodMemberImportSheet: React.FC<
    NeighborhoodMemberImportSheetProps
> = ({ neighborhoodId, open, onOpenChange, onImported }) => {
    const [file, setFile] = useState<File | null>(null);
    const [job, setJob] = useState<ImportJob<NeighborhoodMemberImportPreviewRow> | null>(
        null,
    );
    ...
    const handleDownloadTemplate = async () => {
        try {
            ...
            await downloadNeighborhoodMemberImportTemplate();
        } catch (err) {
    ...
    const handleUpload = async (overrideSheetName?: string) => {
        ...
        try {
            setUploading(true);
            const result = await uploadNeighborhoodMemberImportFile(
                neighborhoodId,
                file,
            ...
            NEIGHBORHOOD_MEMBER_MAPPING_FIELDS.forEach(f => {
    ...
    const handleApplyMapping = async () => {
        if (!job || !mapping.phone || !mapping.roleName) return;
        try {
            setApplying(true);
            const payload: Partial<
                Record<keyof NeighborhoodMemberColumnMapping, string>
            > = {
                phone: mapping.phone,
            ...
            const result = await applyNeighborhoodMemberImportMapping(
                job._id,
                payload as NeighborhoodMemberColumnMapping,
            );
            setJob(result);
            setShowMapping(false);
    ...
    const doCommit = async () => {
        ...
        try {
            setCommitting(true);
            setConfirmOpen(false);
            await commitNeighborhoodMemberImport(job._id);
            const result = await pollImportJobUntilSettled<NeighborhoodMemberImportPreviewRow>(
                job._id,
                setJob,
            );
            ...
            toast.success(`Đã phân công ${result.createdCount} thành viên`);
            reset();
            onOpenChange(false);
            onImported();
        } catch (err) {
    ...
    const canApplyMapping = !!mapping.phone && !!mapping.roleName;
    const canCommit =
        !!job &&
        !showMapping &&
    ...
    return (
        <Sheet open={open} onOpenChange={handleOpenChange}>
            <SheetContent className="flex w-full flex-col sm:max-w-3xl lg:max-w-[calc(100vw-320px)]">
                <SheetHeader>
                    <SheetTitle>Nhập thành viên Tổ từ Excel</SheetTitle>
                </SheetHeader>
                <div className="flex-1 space-y-4 overflow-y-auto py-4">
                    {!job && (
                        <>
                            <div className="rounded-lg border border-divider_01 bg-surface_2 p-3 text-xs text-text_2">
                                ...
                                nào tương ứng với &quot;Số điện thoại&quot;,
                                &quot;Vai trò&quot;... — không cần tên cột phải
                                khớp chính xác. &quot;Số điện thoại&quot; phải
                                khớp với một tài khoản ĐANG HOẠT ĐỘNG đã có sẵn
                                trong hệ thống VÀ đã có vai trò tương ứng (hệ
                                thống không tự tạo tài khoản mới hay gán thêm
                                vai trò từ import này, chỉ phân công phạm vi
                                Tổ). Cộng tác viên nhập qua Excel sẽ mặc định
                                phụ trách toàn bộ Tổ — cần phạm vi hẹp hơn
                                (tuyến đường/nhóm nhà/chiến dịch) thì phân công
                                trực tiếp trên danh sách thành viên.
                            </div>
                            <Button
                                type="button"
                                variant="outline"
                    ...
                    {job && job.availableSheetNames.length > 1 && (
                        <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                            <p>
                                ...
                                Nếu đây không phải sheet chứa dữ liệu thành
                                viên, chọn đúng sheet bên dưới rồi tải lên lại.
                            </p>
                    ...
                    {job && showMapping && (
                        <div className="space-y-4">
                            <div className="rounded-lg border border-divider_01 bg-surface_2 p-3 text-xs text-text_2">
                                ...
                                {job.headers.join(", ")}. Chỉ &quot;Số điện
                                thoại&quot; và &quot;Vai trò&quot; là bắt buộc —
                                &quot;Ghi chú&quot; có thể để &quot;Không
                                dùng&quot; nếu file không có cột tương ứng.
                            </div>

                            {NEIGHBORHOOD_MEMBER_MAPPING_FIELDS.map(f => (
                    ...
                    {job && !showMapping && job.status === "committing" && (
                        <ImportProgressBar
                            ...
                            label="Đang phân công thành viên..."
                        />
                    )}
                ...
                <SheetFooter className="flex-col gap-2">
                    ...
                    {job && !showMapping && (
                        ...
                        >
                            Xác nhận phân công {job.validRows} thành viên
                        </Button>
                    )}
                </SheetFooter>
            </SheetContent>
...
export default NeighborhoodMemberImportSheet;
